    ...
    <View className="flex-1">
      ...
      <FlatList
        ...
        renderItem={({item: CustomerCardProps}) => (
          <CustomerCard {...CustomerCardProps} />
        )}
      ...
      <TouchableOpacity
        className="absolute bottom-2 right-2 bg-pink-600 p-4 rounded-full flex-row items-center shadow-lg"
        onPress={() => navigation.navigate('AddNewCustomer')}>
        <FontAwesomeIcon icon={faPlus} size={16} color={'white'} />
        <Text className="ml-2 text-white font-semibold">Add Customer</Text>
      </TouchableOpacity>
    </View>
  );
};
